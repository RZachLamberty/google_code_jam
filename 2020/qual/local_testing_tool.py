      ...
      if i % 10 == 0:
        with open('log.txt', 'a') as fp:
          ...
          if random.randint(0, 1):
            ...
            fp.write('did reverse\n')
          if random.randint(0, 1):
            self.arr = BitFlip(self.arr)
            fp.write('did bitflip\n')
          fp.write('after {:0>3} steps: {}\n'.format(i, self.arr))
          for (j, c) in enumerate(self.arr, 1):
            fp.write('{}: {}\n'.format(j, c))
          fp.write('\n')
      with open('log.txt', 'a') as fp:
        fp.write('{} --> {}\n'.format(contestant_input,
                                    self.arr[contestant_input - 1]))
